#v1.3 - 200 SMA removed from buy & sell criteria. 
#v1.3 - Exit signal stochastic cross above 50(shorts) or below 50 (longs) 
#v1.3 - Additon of OR operator on exit signal. 
...
#Sell exit signal. Exit when stochastic goes above 50 or if stochastic was less than 20 , exit when goes above. 

df['Sell exit'] = (df['%K'] > 50) | (df['%K'] > 20) & (df['%K'].shift(1) < 20)
...
#Buy exit signal. Exit when stochastic goes below 50 or if stochastic was greater than 80, exit when goes below. 

df['Buy exit'] = (df['%K'] < 50) | (df['%K'] < 80) & (df['%K'].shift(1) > 80)
